...
#include <highgui.h>
            ...
            /*here is a simple program which demonstrates the use of ros and opencv to
            do image manipulations on video streams given out as image topics from the monocular vision
...
class walldetect
  {
      ...
      image_transport::Subscriber image_sub_; //image subscriber // edited
  ...
  public:
      walldetect()
        : it_(nh_)
       {
          ...
          image_sub_ = it_.subscribe("/ardrone/front/image_raw",1, &walldetect::imageCb, this);//edited
          image_pub_= it_.advertise("/arcv/Image",1);//edited
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
       {
        ...
        CvSeq* circles = cvHoughCircles(gray_out, storage,CV_HOUGH_GRADIENT, 2, 1200,1,1 ,0,200);

        for (i = 0; i < circles->total; i++)
            {
              float* p = (float*)cvGetSeqElem( circles, i );
              printf("\n %d ,%d",cvRound(p[0]),cvRound(p[1]));
              if(cvRound(p[0])>=100 && cvRound(p[0])<=300)
                    {
                    if(cvRound(p[1])>=50 && cvRound(p[1])<=100)
                        {
                         printf("\nCircle detected - Avoid obstacle");
                      ...
                      cvCircle( img, cvPoint(cvRound(p[0]),cvRound(p[1])),3, CV_RGB(0,255,0), 3, 8, 0 );
            }


        cvShowImage( "Image",img);
        cvShowImage( "Grayscale image",gray_out);
        cvWaitKey(2);
